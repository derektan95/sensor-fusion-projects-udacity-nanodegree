...
void magnitudeSobel()
{
    ...
    // show result
    cv::namedWindow("Gray Original Image", 1); // create window
    cv::imshow("Gray Original Image", imgGray);
    ...
    // show result
    cv::namedWindow("Gaussian Blur Image", 1); // create window
    cv::imshow("Gaussian Blur Image", blurred);
    cv::waitKey(0); // wait for keyboard input before continuing
